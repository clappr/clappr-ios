...
extension AVFoundationPlayback {
    ...
    open override var isDvrInUse: Bool {
        ...
        guard let currentTime = avPlayer.currentItem?.currentTime().seconds else { return false }
    ...
    open override var currentDate: Date? {
        return avPlayer.currentItem?.currentDate()
    }
    ...
    open override var seekableTimeRanges: [NSValue] {
        guard let ranges = avPlayer.currentItem?.seekableTimeRanges else { return [] }
        return ranges
    }
    
    open override var loadedTimeRanges: [NSValue] {
        guard let ranges = avPlayer.currentItem?.loadedTimeRanges else { return [] }
        return ranges
    }
